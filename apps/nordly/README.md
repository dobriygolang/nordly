...
3. GitHub Actions builds **macOS** (Apple Silicon + Intel) and **Windows**, uploads installers to the GitHub Release, then copies them to **https://trynordly.app/desktop**. Builds ship with cloud APIs on (`VITE_NORDLY_LOCAL_ONLY=false`).
4. The site `/download` and **Settings → Software → Check for Updates** both use `https://trynordly.app/desktop` (`latest.json` / `releases.json`).

**One-time manual install:** download from https://trynordly.app/download after the first `nordly-v*` tag lands on the CDN.
...
- CDN upload (`DEPLOY_SSH_*`) is required on every tagged desktop release so `/download` and in-app updates share `https://trynordly.app/desktop`.
