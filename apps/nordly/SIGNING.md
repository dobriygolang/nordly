...
2. Right-click the **private key** → **Export** → save as `nordly-developer-id.p12` with a password.
3. Base64 for GitHub:

```bash
openssl base64 -A -in nordly-developer-id.p12 -out nordly-developer-id.b64
# Paste nordly-developer-id.b64 → secret APPLE_CERTIFICATE
...
5. **`sync-cdn`** — downloads release assets, rewrites updater URLs to `trynordly.app/desktop`, SCP to VPS. Required for every tagged desktop release (`DEPLOY_SSH_*`).
...
CDN secrets (`DEPLOY_SSH_*`) are required on every desktop release.
