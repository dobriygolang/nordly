...
// через `localStorage['nordly:ambient-url']`. Юзер сам не настраивает.
//
...
const PERSIST_KEY = 'nordly:ambient:enabled';
const URL_OVERRIDE_KEY = 'nordly:ambient-url';
...
// override'ится через `localStorage['nordly:ambient-url']` если юзер хочет
 ...
 * включил ambient (`nordly:ambient:enabled=1`), пытается start. На autoplay
