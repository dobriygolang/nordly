...
import {
  ...
  tasksByDaySignature,
  type ColumnItems,
} from '@features/tasks/lib/dayTaskDndUtils';
...
const EMPTY_NO_DROP_KEYS: ReadonlySet<string> = new Set();
...
export function useDayTaskDnd({
  ...
  useEffect(() => {
    ...
    const onVisibility = (): void => {
      if (document.hidden) stopDragLoop();
      else startDragLoop();
    };
