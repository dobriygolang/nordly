...
function hitTestAt(
  ...
  const col = section instanceof HTMLElement ? section : null;
...
function createGhost(fromEl: HTMLElement): HTMLElement {
  ...
  ghost.style.background = light ? 'rgb(255 255 255 / 0.96)' : 'rgb(22 22 22 / 0.96)';
  ghost.style.border = light ? '1px solid rgb(15 15 15 / 0.1)' : '1px solid rgb(255 255 255 / 0.12)';
  ghost.style.borderRadius = '12px';
  ghost.style.boxShadow = light
    ? '0 14px 34px -16px rgb(15 15 15 / 0.28), 0 4px 14px -8px rgb(15 15 15 / 0.16)'
    : '0 16px 40px -10px rgb(0 0 0 / 0.5)';
  ghost.style.transformOrigin = 'top left';
  ghost.style.willChange = 'transform';
  ghost.style.setProperty('--nordly-drag-x', '0px');
  ghost.style.setProperty('--nordly-drag-y', '0px');
  // Tactile pickup: start slightly shrunk, then lift + tilt on the next frame.
  ghost.style.transform =
    'translate3d(var(--nordly-drag-x), var(--nordly-drag-y), 0) scale(0.97)';
  ghost.style.transition = 'transform 150ms cubic-bezier(0.2, 0.7, 0.2, 1)';
  document.body.appendChild(ghost);
  requestAnimationFrame(() => {
    ghost.style.transform =
      'translate3d(var(--nordly-drag-x), var(--nordly-drag-y), 0) scale(1.03) rotate(1.5deg)';
    window.setTimeout(() => {
      if (ghost.isConnected && ghost.dataset.dragDismissing !== 'true') {
...
function dismissGhost(ghost: HTMLElement): void {
  ghost.dataset.dragDismissing = 'true';
  ghost.style.transition =
    'transform 100ms cubic-bezier(0.2, 0.7, 0.2, 1), opacity 100ms cubic-bezier(0.2, 0.7, 0.2, 1)';
  ghost.style.transform =
    'translate3d(var(--nordly-drag-x), var(--nordly-drag-y), 0) scale(1) rotate(0deg)';
...
function moveGhost(ghost: HTMLElement, x: number, y: number, offsetX: number, offsetY: number): void {
  ghost.style.setProperty('--nordly-drag-x', `${x - offsetX}px`);
  ghost.style.setProperty('--nordly-drag-y', `${y - offsetY}px`);
}
...
export function useDayTaskDrag(
  ...
  const dropTargetRef = useRef<DropTarget>({ dayKey: null, insertBeforeTaskId: null });
  ...
  const releaseSession = useCallback((s: DragSession) => {
    ...
    dropTargetRef.current = { dayKey: null, insertBeforeTaskId: null };
    clearTextSelection();
  }, []);
  ...
  useEffect(() => {
    const syncDropTarget = (next: DropTarget) => {
      const prev = dropTargetRef.current;
      if (prev.dayKey === next.dayKey && prev.insertBeforeTaskId === next.insertBeforeTaskId) return;
      dropTargetRef.current = next;
      setDropDay(next.dayKey);
      setDropInsertBeforeId(next.insertBeforeTaskId);
    };
    ...
    const onMove = (e: PointerEvent) => {
      ...
      syncDropTarget({ dayKey, insertBeforeTaskId });
    };
