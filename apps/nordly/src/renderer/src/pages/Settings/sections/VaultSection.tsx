...
export function VaultSection() {
  ...
  const statusLabel = !enabled
    ? t('nordly.settings.vault.status_off')
    : unlocked
      ? t('nordly.settings.vault.status_unlocked')
      : t('nordly.settings.vault.status_locked');

  return (
    <>
      <SettingsGroup title={t('nordly.settings.section.vault')}>
        <SettingRow label={t('nordly.settings.vault.label')} hint={t('nordly.settings.vault.hint')}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
            <Toggle
              value={enabled}
              onChange={(v) => void handleToggle(v)}
              label={enabled ? t('nordly.settings.vault.on') : t('nordly.settings.vault.off')}
            />
        ...
        {enabled && !unlocked && (
          <SettingRow label={t('nordly.settings.vault.unlock_label')} hint={t('nordly.settings.vault.unlock_hint')}>
            <button
              type="button"
        ...
        {enabled && unlocked && (
          <SettingRow label={t('nordly.settings.vault.lock_label')} hint={t('nordly.settings.vault.lock_hint')}>
            <button type="button" className="nordly-settings-vault-btn" onClick={() => lockVault()}>
              {t('nordly.settings.vault.lock_now')}
            </button>
          </SettingRow>
        )}
        ...
        {(enabled || hasRecovery) && (
          <SettingRow
            label={t('nordly.settings.vault.recovery_label')}
            hint={t('nordly.settings.vault.recovery_hint')}
          >
            <button
              type="button"
              className="nordly-settings-vault-btn"
            ...
            >
              {t('nordly.settings.vault.recovery_cta')}
            </button>
          </SettingRow>
        )}
      </SettingsGroup>
            ...
            {modal.kind === 'show-recovery' && (
              <>
                ...
                <label className="nordly-vault-modal__check">
                  ...
                  />
                  {t('nordly.settings.vault.recovery_saved_ack')}
                </label>
            ...
            {modal.kind === 'recovery' && (
              <>
                ...
                <textarea
                  ...
                  placeholder={t('nordly.settings.vault.recovery_placeholder')}
                  rows={4}
                />
                {error && <p className="nordly-vault-modal__error mono">{error}</p>}
                <button
                ...
                >
                  {busy ? t('nordly.vault.cta.working') : t('nordly.settings.vault.recovery_submit')}
                </button>
              </>
            )}
          </div>
...
function VaultPassForm({
  ...
  return (
    <form
      ...
      className="nordly-vault-modal__form"
    >
      <input
        type="password"
      ...
      {error && <p className="nordly-vault-modal__error mono">{error}</p>}
      <button type="submit" className="nordly-vault-modal__primary" disabled={busy}>
        {busy ? t('nordly.vault.cta.working') : t('nordly.vault.cta.unlock')}
      </button>
    </form>
  );
}
