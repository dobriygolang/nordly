...
//
// Daily goal живёт в localStorage `nordly:daily-focus-goal-min` (default 120).
  ...
  return (
    ...
    >
      ...
      <SimpleStatCell
        ...
        sub={
          totalSessionsAll > 0
            ? t('nordly.stats.sessions_total', { n: totalSessionsAll })
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
          ...
          <span style={{ fontSize: 10, color: 'var(--ink-40)' }}>{t('nordly.stats.days_of_goal', { goal })}</span>
        </div>
        <div style={{ fontSize: 10, color: 'var(--ink-40)' }}>{t('nordly.stats.streak_goal')}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <div style={BASELINE_ROW}>
        ...
        <span style={{ fontSize: 10, color: 'var(--ink-40)' }}>{t('nordly.stats.hrs')}</span>
      ...
      <div style={{ fontSize: 10, color: 'var(--ink-40)' }}>
        {t('nordly.stats.vs_last_week', { hrs: prevHrs.toFixed(1) })}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <div style={BASELINE_ROW}>
        ...
        <span style={{ fontSize: 10, color: 'var(--ink-40)' }}>{t('nordly.stats.min_today', { goal: goalMin })}</span>
      </div>
      <div
        aria-hidden
        style={{
      ...
      <div style={{ fontSize: 10, color: 'var(--ink-40)' }}>{t('nordly.stats.daily_goal')}</div>
    </div>
  );
}
