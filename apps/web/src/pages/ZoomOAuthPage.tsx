...
function buildDeepLink(status: string, detail: string | null): string {
  const q = new URLSearchParams({ zoom: status })
  ...
  if (!status) {
    return (
      <main className="mx-auto flex min-h-[60vh] max-w-lg flex-col justify-center px-6 py-16 text-center">
        <h1 className="text-xl font-semibold">{t('oauth.zoom.missing')}</h1>
  ...
  return (
    <main className="mx-auto flex min-h-[60vh] max-w-lg flex-col justify-center px-6 py-16 text-center">
      ...
      <h1 className="mt-2 text-2xl font-semibold">
        {ok ? t('oauth.zoom.successTitle') : t('oauth.zoom.errorTitle')}
      </h1>
      <p className="mt-3 text-sm text-neutral-600 dark:text-neutral-400">
        {ok ? t('oauth.zoom.successBody') : detail ?? t('oauth.zoom.errorBody')}
      </p>
      {deepLink ? (
        <a
          href={deepLink}
        ...
        >
          {t('oauth.zoom.openApp')}
        </a>
      ) : null}
      <p className="mt-6 text-xs text-neutral-500">{t('oauth.zoom.fallbackHint')}</p>
    </main>
  )
}
