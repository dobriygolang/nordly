...
Downstream consumers polled the interview internal API every ~2s:

```
interview.domain_outbox
  ← ClaimOutboxEvents (recommendation: 4 event types)
  ← ClaimOutboxEvents (attempt_submitted — removed with interview stack)
```
...
recommendation (and other retired consumers)
...
| Subject | Historical consumer |
|---------|---------------------|
| `interview.attempt_submitted` | removed with interview stack |
...
3. **Remove poll workers** from retired consumers; keep internal Ack RPC or mark rows published in relay only.

## Idempotency

Keep `processed_events(consumer, event_id)` in recommendation. Bus delivery is at-least-once.

## Ops

- Relay is single-writer to outbox claims — no race with consumer poll (today's multi-consumer split stays until cutover).
...
Prod (`deploy/docker-compose.prod.yml`): `nats` service + `OUTBOX_RELAY_ENABLED=true` on interview, `OUTBOX_POLL_ENABLED=false` on recommendation (historical).
