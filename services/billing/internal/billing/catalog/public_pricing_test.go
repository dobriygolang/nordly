...
func TestPublicPricingViewHidesInternalEntitlements(t *testing.T) {
	...
	item := catalog.PlanCatalogItem{
		...
		Limits: map[string]catalog.PlanLimitSpec{
			...
			model.EntitlementPublishedNotesActive: {Type: "gauge", Limit: intPtr(10)},
			...
			model.EntitlementCloudSyncDevices:     {Type: "gauge", Limit: intPtr(0)},
		},
	...
	if _, ok := out.Limits[model.EntitlementCloudNotesCount]; ok {
		t.Fatal("cloud_notes_count should not appear in public pricing catalog")
	}
	if _, ok := out.Limits[model.EntitlementPublishedNotesActive]; !ok {
		t.Fatal("published_notes_active should appear in public pricing catalog")
	}
	if _, ok := out.Limits[model.EntitlementCloudSyncDevices]; !ok {
		t.Fatal("cloud_sync_devices should appear in public pricing catalog")
	}
